...
async function uploadAndProcessNote(
  ...
  while (Date.now() - start < 15000) {
    ...
    if (statusRes.data.data.status === 'COMPLETED' || statusRes.data.data.status === 'FAILED') {
      break;
    }
...
async function runPhase4Tests() {
  console.log('================================================================');
  console.log('🧪 CAMPUSNOTES AI — PHASE 4 RAG CHATBOT VERIFICATION SUITE');
  console.log('   Testing Grounded Generation, Citations, Sessions & Security');
  console.log('================================================================\n');
  ...
  try {
    ...
    console.log('--- Setting Up Academic Test Documents ---');
    ...
    // -------------------------------------------------------------------------
    // TEST 1: Question with an obvious answer in a PDF
    // -------------------------------------------------------------------------
    console.log('\n--- Test 1: Obvious Question Grounded in PDF ---');
    ...
    assert(
      mentionsDeadlockOrSafety && hasCorrectSource,
      'Obvious question answered accurately and grounded in Page 2 of OS Note',
      `Answer excerpt: "${q1Data.answer.slice(0, 100)}..." | Top Similarity: ${q1Data.topSimilarity} | Chunks: ${q1Data.retrievedCount}`
    );
    ...
    // -------------------------------------------------------------------------
    // TEST 14: Phase 1–3B Regression Verification
    // -------------------------------------------------------------------------
    console.log('\n--- Test 14: Phase 1–3B End-to-End Regression ---');
    ...
    console.error('Unexpected error during Phase 4 verification:', err.response?.data || err.message);
    failed++;
  } finally {
    // Cleanup sessions
